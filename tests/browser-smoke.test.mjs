...
const mockDoc = {
  ...
  querySelector: (sel) => {
    ...
    if (sel.includes('input[')) return { value: '4', checked: true };
...
vm.runInContext(
  testsJsCode + '\n;globalThis.TESTS_REF = TESTS; globalThis.currentLang = currentLang; globalThis.activeTest = activeTest; globalThis.scorePublicRouteV2 = typeof scorePublicRouteV2 !== "undefined" ? scorePublicRouteV2 : null;',
  sandbox
);
...
assert.equal(typeof sandbox.scorePublicRouteV2, 'function', 'scorePublicRouteV2 must be a function');

// Run scorePublicRouteV2 in sandbox
await sandbox.scorePublicRouteV2();

console.log('Browser smoke test passed successfully! Interface & scoring functions are fully restored.');
