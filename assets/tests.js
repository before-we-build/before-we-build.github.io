...
function positionTypes(test,calc){const code=x=>test.mode==='position'&&activeTest==='psychosophy'?test.code[x]:(test.code[currentLang]?.[x]||test.code.en[x]),psyEn={Воля:'V',Логика:'L',Эмоция:'E',Физика:'F'};return permutations(test.aspects).map(order=>{const raw=order.reduce((s,a,i)=>s+(calc.scores[`${a}|${i+1}`]||0)/(calc.counts[`${a}|${i+1}`]||1),0);const display=order.map(code).join(activeTest==='temporistics'&&currentLang==='uk'?'-':'');const en=activeTest==='temporistics'?order.map(a=>test.code.en[a]).join(''):(activeTest==='psychosophy'?order.map(a=>psyEn[a]).join(''):display);return{display,en,positions:order.map((a,i)=>`${i+1}${activeTest==='temporistics'?test.code.en[a]:test.code[a]}`),raw}}).sort((a,b)=>b.raw-a.raw).slice(0,3)}
...
function isQuestionBank(bank){return bank?.schemaVersion==='1.0.0'&&bank.tests&&['socionics','psychosophy','temporistics'].every(key=>{const test=bank.tests[key];return test&&Array.isArray(test.items)&&test.items.length>0&&test.labels&&test.mode})}
...
async function loadQuestionBank(){try{const response=await fetch(QUESTION_BANK_URL,{cache:'no-store'});if(!response.ok)throw new Error(`Question bank request failed: ${response.status}`);const bank=parseQuestionBank(await response.text());if(!isQuestionBank(bank))throw new Error('Question bank schema is invalid');TESTS=bank.tests;window.beforeWeBuildQuestionBankVersion=bank.bankVersion||null}catch(error){console.warn('Using the bundled test fallback because the research question bank could not be loaded.',error)}}
...
function resultLinks(top=[]){const d=I18N[currentLang],system={socionics:'socionics-overview',psychosophy:'psychosophy-overview',temporistics:'temporistics-overview'}[activeTest],note={socionics:d.levelNoteSoc,psychosophy:d.levelNotePsy,temporistics:d.levelNoteTmp}[activeTest],typeLinks=top.map(t=>{const code=t.code||t.en||t.display,slug=activeTest==='socionics'?SOC_PAGES[t.code]:`${activeTest}-type-${(t.en||t.display).toLowerCase()}`;return`<a href="${wikiEntityLang(slug)}" target="_blank" rel="noopener">${d.readType}: ${code}</a>`}).join('');return`<div class="result-reading"><h4>${d.insideModel}</h4><p class="test-caveat">${note} ${d.insideCaveat}</p><h4>${d.readNext}</h4><div class="result-links"><a href="${wikiConcept('test-result-reading-guide')}" target="_blank" rel="noopener">${d.readGuide}</a><a href="${wikiConcept('compatibility-level-boundaries')}" target="_blank" rel="noopener">${d.readBoundaries}</a><a href="${wikiEntityLang(system)}" target="_blank" rel="noopener">${d.readSystem}</a>${typeLinks}</div></div>`}
async function buildPayload(calc,rows,summary){return{...STUDY,instrumentVersion:TESTS[activeTest].version,responseId:crypto.randomUUID(),sessionId,deviceRespondentId:deviceId,retestTokenHash:await hash(val('#retestToken')),randomization:{seed:deviceId,itemOrder:calc.responses.map(r=>r.itemId)},timing:{startedAt:new Date(startedAt).toISOString(),completedAt:new Date().toISOString(),totalDurationMs:Date.now()-startedAt},metadata:{language:currentLang,ageBand:val('#ageBand'),priorTypologyExposure:val('#exposure'),selfReportedType:val('#selfType'),viewport:{w:innerWidth,h:innerHeight},timezone:Intl.DateTimeFormat().resolvedOptions().timeZone},responses:calc.responses,scaleScores:Object.fromEntries(rows),resultSummary:summary,qualityFlags:{failedAttentionCheck:calc.failedAttention,straightlining:new Set(calc.responses.filter(r=>!r.attentionExpected).map(r=>r.responseValue)).size===1,tooFast:(Date.now()-startedAt)<45000}}}
async function scoreActiveTest(){const d=I18N[currentLang],out=document.querySelector('#testResult'),test=TESTS[activeTest],consent=document.querySelector('#consent')?.checked,calc=collect();if(calc.missing||!consent){out.innerHTML=`<div class="test-result"><p class="test-caveat">${d.missing}</p></div>`;return}const rows=rowsFor(test,calc);let main='',summary='',cl='low',labels=SOC_LABELS[currentLang]||SOC_LABELS.ru;if(test.mode==='socionics'){const top=socionics(calc),max=top[0].raw;cl=clarity(top,calc.responses);summary=`${d.topTIM}: ${top.map(t=>`${t.code} (≈ ${t.mbti})`).join(', ')}. ${d.clarity}: ${d[cl]}.`;main=`<h4>${d.topTIM}</h4><div class="tim-list">${top.map((t,i)=>`<div class="tim-card"><b>${i+1}. ${t.code} — ${t.name} (≈ ${t.mbti})</b><span class="bar-track"><span class="bar-fill" style="width:${Math.round(t.raw/max*100)}%"></span></span></div>`).join('')}</div>${resultLinks(top)}<h4>${d.aspectTrace}</h4>`}else{const top=positionTypes(test,calc),max=top[0].raw;cl=clarity(top,calc.responses);summary=`${d.topType}: ${top.map(t=>t.display+(activeTest==='temporistics'&&currentLang!=='en'?` (${t.en})`:'')).join(', ')}. ${d.clarity}: ${d[cl]}.`;const music=await musicRecommendation(top);main=`<h4>${d.topType}</h4><div class="tim-list">${top.map((t,i)=>`<div class="tim-card"><b>${i+1}. ${t.display}${activeTest==='temporistics'&&currentLang!=='en'?` · ${t.en}`:''}</b><small>${t.positions.join(' · ')}</small><span class="bar-track"><span class="bar-fill" style="width:${Math.round(t.raw/max*100)}%"></span></span></div>`).join('')}</div>${resultLinks(top)}${music}<h4>${d.scaleTrace}</h4>`}const payload=await buildPayload(calc,rows,summary);localStorage.setItem('before-we-build-results',JSON.stringify(payload));out.innerHTML=`<div class="test-result">${main}<p><b>${d.clarity}:</b> ${d[cl]}</p><div class="result-bars">${rows.map(([k,v])=>`<div class="result-row"><b>${labels[k]||k.replace('|','')}</b><span class="bar-track"><span class="bar-fill" style="width:${Math.round(v/5*100)}%"></span></span><em>${v.toFixed(1)}</em></div>`).join('')}</div><p class="test-caveat">${test.mode==='socionics'?d.caveatSoc:d.caveatPosition}</p><div class="test-actions"><button data-export>${d.export}</button><button data-copy>${d.copy}</button></div></div>`;out.querySelector('[data-export]').onclick=()=>download(payload);out.querySelector('[data-copy]').onclick=()=>navigator.clipboard?.writeText(summary)}
...
function renderGameItem(it,i,total){const kind=gameKind(it,i),copy=gameCopy(kind,currentLang),scene=scenePrompt(it),labels=copy.labels,raw=it.text[currentLang]||it.text.ru||it.text.en;shownAt[it.id]=Date.now();return`<article class="test-item story-test-card ${kind} ${i===publicStep?'active-step':''}" data-game="${kind}" data-test-item="${it.id}" data-test-key="${it.testKey||activeTest}" data-display-index="${i+1}"><div class="story-card-top"><span>${i+1}/${total}</span><span>${escapeHtml(copy.title)}</span></div><p class="story-scene">${escapeHtml(scene.scene)}</p><p class="story-hint">${escapeHtml(scene.hint)}</p><details class="story-meaning"><summary>${I18N[currentLang].publicWhyQuestion}</summary><p>${escapeHtml(raw)}</p></details><div class="game-visual" aria-hidden="true"><span></span><span></span><span></span></div><div class="scale game-scale ${gameControlClass(kind)}" role="radiogroup" aria-label="${escapeHtml(raw)}">${[1,2,3,4,5].map(v=>`<label class="game-choice game-choice-${v}"><input type="radio" name="${it.id}" value="${v}"><span><i>${gameIcon(kind,v)}</i><em>${escapeHtml(labels[v-1])}</em></span></label>`).join('')}</div><p class="story-feedback" aria-live="polite"></p></article>`}
...
function publicItems(){return shuffle(Object.entries(TESTS).flatMap(([testKey,t])=>t.items.filter(it=>!it.attention).map(it=>({...it,testKey}))),deviceId+'public-mixed')}
function publicModeForAge(ageBand){return ageBand==='under-18'?'visual':'classic'}
function renderPublicAgeSelect(d){const ages=[['under-18',d.publicAgeUnder18],['18-24','18–24'],['25-34','25–34'],['35-44','35–44'],['45-54','45–54'],['55+','55+']];return`<label class="public-age-field"><span>${d.publicAgeLabel}</span><select data-public-age><option value="">${d.publicAgePlaceholder}</option>${ages.map(([value,label])=>`<option value="${value}" ${publicAgeBand===value?'selected':''}>${label}</option>`).join('')}</select></label><p class="test-caveat public-age-error" data-public-age-error hidden>${d.publicAgeRequired}</p>`}
function renderPublicStart(d){return`<div class="public-start"><div class="public-start-card"><p class="eyebrow">${d.publicStartEyebrow}</p><h2>${d.publicStartTitle}</h2><p>${d.publicStartText}</p>${renderPublicAgeSelect(d)}<div class="test-actions public-start-actions"><button class="primary" data-public-start>${d.publicStartButton}</button><span class="privacy-tooltip"><button class="privacy-help" aria-label="${d.publicStartPrivacyLabel}" aria-describedby="public-privacy-tip">?</button><span id="public-privacy-tip" role="tooltip">${d.publicStartPrivacyNote}</span></span></div></div></div>`}
function renderClassicPublicItem(it,i,total){shownAt[it.id]=Date.now();const text=it.text[currentLang]||it.text.ru||it.text.en;return`<article class="test-item public-classic-item ${i===publicStep?'active-step':''}" data-test-item="${it.id}" data-test-key="${it.testKey}" data-display-index="${i+1}"><p><b>${i+1}/${total}.</b> ${escapeHtml(text)}</p><div class="scale" role="radiogroup" aria-label="${escapeHtml(text)}">${[1,2,3,4,5].map(v=>`<label><input type="radio" name="${it.id}" value="${v}">${v}</label>`).join('')}</div><div class="scale-help"><span>${I18N[currentLang].scaleL}</span><span>${I18N[currentLang].scaleR}</span></div></article>`}
...
function setPublicStep(step){const cards=[...document.querySelectorAll('.story-test-card,.public-classic-item')];if(!cards.length)return;publicStep=Math.max(0,Math.min(step,cards.length-1));cards.forEach((card,i)=>card.classList.toggle('active-step',i===publicStep));document.querySelector('[data-public-prev]')?.toggleAttribute('disabled',publicStep===0);document.querySelector('[data-public-next]')?.toggleAttribute('disabled',publicStep===cards.length-1);const active=cards[publicStep];if(active&&!shouldReduceMotion())active.scrollIntoView({block:'center',behavior:'smooth'})}
function revealFirstMissingPublicCard(){const missing=document.querySelector('.story-test-card.missing-answer,.public-classic-item.missing-answer');if(!missing)return;const cards=[...document.querySelectorAll('.story-test-card,.public-classic-item')],idx=cards.indexOf(missing);if(idx>=0)setPublicStep(idx)}
function setPublicPaused(paused){document.querySelector('[data-public-flow]')?.toggleAttribute('hidden',paused);document.querySelector('[data-public-pause-card]')?.toggleAttribute('hidden',!paused)}
function renderTests(){
  ...
  if(!isResearch){
    if(!publicStarted){panel.innerHTML=renderPublicStart(d);const ageSelect=panel.querySelector('[data-public-age]'),ageError=panel.querySelector('[data-public-age-error]');ageSelect?.addEventListener('change',()=>{publicAgeBand=ageSelect.value;ageError?.toggleAttribute('hidden',!!publicAgeBand)});panel.querySelector('[data-public-start]')?.addEventListener('click',()=>{publicAgeBand=ageSelect?.value||'';if(!publicAgeBand){ageError?.removeAttribute('hidden');ageSelect?.focus();return}publicMode=publicModeForAge(publicAgeBand);publicSaveLocal=false;const card=panel.querySelector('.public-start-card');card?.classList.add('is-leaving');setTimeout(()=>{publicStarted=true;publicStep=0;startedAt=Date.now();renderTests()},shouldReduceMotion()?0:220)});return}
    const items=publicItems();shownAt={};changed={};
    ...
    panel.innerHTML=`<div data-public-flow><div class="story-progress" role="status" aria-live="polite"><div><b>${d.publicProgress}</b><span data-story-progress-count>0/${items.length}</span></div><span><i data-story-progress-bar></i></span></div><div class="test-items public-step-list ${itemsClass}">${itemHtml}</div><div class="public-step-nav"><button data-public-prev>${d.publicPrev}</button><button data-public-next>${d.publicNext}</button></div><div class="test-actions"><button class="primary" data-submit>${d.submit}</button><button data-public-pause>${d.publicPause}</button><button data-reset>${d.reset}</button><button data-clear>${d.clear}</button></div><div id="testResult" aria-live="polite"></div></div><div class="public-pause-card" data-public-pause-card hidden><h3>${d.publicPausedTitle}</h3><p>${d.publicPausedText}</p><button class="primary" data-public-resume>${d.publicResume}</button></div>`;
    updateStoryProgress();
    bindPublicAnswers(panel);
    setPublicStep(publicStep);
    panel.querySelector('[data-public-prev]')?.addEventListener('click',()=>setPublicStep(publicStep-1));
    panel.querySelector('[data-public-next]')?.addEventListener('click',()=>setPublicStep(publicStep+1));
    ...
    panel.querySelector('[data-reset]')?.addEventListener('click',()=>{publicStarted=false;publicStep=0;renderTests()});
    panel.querySelector('[data-clear]')?.addEventListener('click',()=>{localStorage.removeItem('before-we-build-results');lastPublicPayload=null;document.querySelector('#testResult').innerHTML=`<div class="test-result"><p>${d.publicCleared}</p></div>`});
    return;
  }
  ...
  if(!consentAccepted){
    shownAt={};changed={};tabs.innerHTML='';
    ...
    const itemHtml=isResearch?items.map((it,i)=>{shownAt[it.id]=Date.now();return`<article class="test-item" data-test-item="${it.id}" data-display-index="${i+1}"><p><b>${i+1}.</b> ${it.text[currentLang]||it.text.ru}</p><div class="scale">${[1,2,3,4,5].map(v=>`<label><input type="radio" name="${it.id}" value="${v}">${v}</label>`).join('')}</div><div class="scale-help"><span>${d.scaleL}</span><span>${d.scaleR}</span></div></article>`}).join(''):items.map((it,i)=>renderGameItem(it,i,items.length)).join('');
    ...
    panel.innerHTML=`${consentCard}${metadata}<div class="test-meta ${isResearch?'':'story-test-meta'}"><h3>${test.labels[currentLang][0]}</h3><p>${test.labels[currentLang][1]}</p>${caveat}</div>${storyProgress}<div class="test-items ${isResearch?'':'story-feed'}">${itemHtml}</div><div class="test-actions"><button class="primary" data-submit>${d.submit}</button><button data-reset>${d.reset}</button>${clearButton}</div><div id="testResult" aria-live="polite"></div>`;
  }
  tabs.querySelectorAll('[data-test-key]').forEach(b=>b.addEventListener('click',()=>{activeTest=b.dataset.testKey;renderTests()}));
  panel.querySelector('#consent')?.addEventListener('change',e=>{window.beforeWeBuildConsentAccepted=e.target.checked;renderTests()});
  updateStoryProgress();
  panel.querySelectorAll('input[type="radio"]').forEach(i=>i.addEventListener('change',()=>{changed[i.name]=true;const card=i.closest('.story-test-card');card?.classList.add('answered');const label=i.closest('label')?.querySelector('em')?.textContent||'';const fb=card?.querySelector('.story-feedback');if(fb)fb.textContent=label?`Выбрано: ${label}`:'';updateStoryProgress();const next=card?.nextElementSibling;if(next?.classList.contains('story-test-card'))setTimeout(()=>next.scrollIntoView({block:'center',behavior:'smooth'}),160)}));
  panel.querySelector('[data-submit]')?.addEventListener('click',async()=>{await scoreActiveTest();if(!isResearch)document.querySelector('#testResult [data-export]')?.remove();const payload=()=>JSON.parse(localStorage.getItem('before-we-build-results')||'null');document.querySelector('#testResult [data-export]')?.addEventListener('click',()=>{const p=payload();if(p)download(p)});document.querySelector('#testResult [data-copy]')?.addEventListener('click',()=>navigator.clipboard?.writeText(payload()?.resultSummary||''))});
