...
function savePublicProgress(){if(!publicSaveLocal||!publicStarted)return;localStorage.setItem(PUBLIC_PROGRESS_KEY,JSON.stringify({schemaVersion:1,questionBankVersion,testKeys:publicTestKeys,orderSeed:publicOrderSeed,step:publicStep,startedAt,selectedAnswers,shownAt,answered,changed,answerTiming}))}
function clearPublicProgress(){localStorage.removeItem(PUBLIC_PROGRESS_KEY)}
function restorePublicProgress(progress){publicTestKeys=normalizedPublicTestKeys(progress.testKeys);publicOrderSeed=progress.orderSeed||sessionId;publicStep=Math.max(0,Number(progress.step)||0);startedAt=Number(progress.startedAt)||Date.now();selectedAnswers=progress.selectedAnswers||{};shownAt=progress.shownAt||{};answered=progress.answered||{};changed=progress.changed||{};answerTiming=progress.answerTiming||{};publicSaveLocal=true;publicStarted=true;renderTests()}
...
function renderTests(){
  ...
  if(!isResearch){
    if(!publicStarted){
      panel.innerHTML=renderPublicStart(d);const saveControl=panel.querySelector('[data-public-save]');
      panel.querySelectorAll('[data-public-route]').forEach(button=>button.addEventListener('click',()=>{publicTestKeys=button.dataset.publicRoute==='all'?Object.keys(TESTS):[button.dataset.publicRoute];publicSaveLocal=!!saveControl?.checked;publicOrderSeed=`${sessionId}:${publicTestKeys.join('-')}`;resetAnswerState();publicStarted=true;publicStep=0;startedAt=Date.now();savePublicProgress();renderTests()}));
    ...
    const itemHtml=items.map((it,i)=>renderClassicPublicItem(it,i,items.length)).join('');
    const itemsClass='public-classic-list';
